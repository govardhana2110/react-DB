...
import { Box, Container, Button, Tooltip, Typography } from '@mui/material';
...
const Balance = () => {
  ...
  const changeMSISDN = (evt) => {
    setMsisdn(evt.target.value);
  };
  const setDateTime = () => {
  ...
  const onBalanceSubmit = async (evt) => {
    const dateFormate = moment(`${theDate.date} ${theDate.time}`).format(
      'MMDDHHmmYYYY.ss',
    );

    const inputVal = {
      command: `perl demo_change_pvt.pl -i ${msisdn} `,
      status: '0',
    };
    try {
